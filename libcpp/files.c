...
static bool libfs_enable = false;
...
void cpp_init_libfs (void * lfs_buf){
  int flag;
  if(lfs_buf==NULL){
    lfs_buf = xmalloc(512*1024*1024);
    lfs_buf = (void *)((unsigned long)lfs_buf & 0xFFFFFFFFFFFFF000);
    lfs_buf += 0x01000;
    flag = LFS_FORMAT;
  }
  else{
    flag = LFS_INIT;
    long ret_t4 = syscall(334, "newm", 4, 2);
    //printf("Result of attach: %d\n", ret_t4);
    lfs_buf = (void *)PBRKSTART;
  }
  lfs_init(lfs_buf, flag, 256*1024*1024);
  char namebuf[1024];
  memset(namebuf, 0, 1024);
  // lfs_dump(1, namebuf, 0);
  /*if(flag == LFS_INIT) {
...
open_file (_cpp_file *file)
{
  ...
  if (libfs_enable){
    //printf("opening: %s\n", file->path);
      if(file->path[0]!='/'){
        char nn[1024];
        snprintf(nn,1024,"/%s",file->path);
        //printf("opening: %s\n", nn);
        file->fd = lfs_open (nn, LFS_O_RDONLY);
        //printf("opened: %s %d\n", nn, file->fd);
      }
      else{
        file->fd = lfs_open (file->path, LFS_O_RDONLY);
        char buf;
        //printf("opened: %d\n", file->fd);
      }
  ...
  if (file->fd != -1)
    {
      if (libfs_enable){
        ...
        ret = fstat (file->fd, &file->st_);
      if (ret == 0)
	{
    
	  if (1) //!S_ISDIR (libfs_enable ? file->st.st_mode : file->st_.st_mode)
	    {
	      file->err_no = 0;
        //printf("returned open!%s\n", file->path);
	      return true;
	     }
...
read_file_guts (cpp_reader *pfile, _cpp_file *file, source_location loc)
{
  ...
  if (S_ISBLK (0))
    {
      cpp_error_at (pfile, CPP_DL_ERROR, loc,
		    "%s is a block device, %x", file->path, libfs_enable ? file->st.st_mode : file->st_.st_mode);
      return false;
    }
  ...
  while ((count = 
  ( libfs_enable ? 
  lfs_read (file->fd, buf + total, size - total) : 
  read (file->fd, buf + total, size - total)
  ) ) 
  > 0)
    {
      //libfs_enable ? printf("***reading file: %s, count: %d, total: %d, error: %d\n",file->path, count, size, lfs_error):0;
      total += count;
  ...
  //printf("***end reading file: %s, count: %d, error: %d\n",file->path, count, lfs_error);
  if (count < 0)
    {
...
read_file (cpp_reader *pfile, _cpp_file *file, source_location loc)
{
  ...
  libfs_enable ? lfs_close (file->fd) : close (file->fd);
  //printf("close: %s\n", file->path);
  file->fd = -1;
		   ...
		   source_location loc)
{
  ...
  for (f = pfile->all_files; f; f = f->next_file)
	...
	continue;
	  ...
	  && f->err_no == 0
	  && f->st.st_modtime == file->st.st_modtime
			...
			int angle_brackets)
{
  ...
  return file->st.st_modtime > pfile->buffer->file->st.st_modtime;
}
...
{
  file->st_.st_ino = (ino_t)file->st.st_ino;
  file->st_.st_mode = (mode_t)file->st.st_mode;
  file->st_.st_nlink = (nlink_t)file->st.st_nlink;
  file->st_.st_uid = file->st.st_uid;
  file->st_.st_gid = file->st.st_gid;
  file->st_.st_size = file->st.st_size;
  file->st_.st_mtime = file->st.st_modtime;
    return &file->st_;
}
